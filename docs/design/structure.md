...
| `tsdown.config.ts` | `entry: { index: "src/index.ts", cli: "src/cli/main.ts" }`, `format: "esm"`, `dts: true`, `deps: { neverBundle: ["yaml", "better-result"] }`, shebang on the `cli` entry. `neverBundle` is mandatory — the default bundles dependencies and the toolchain doc measured 234 kB of `yaml` inlined. `better-result` in that list is INFERRED but forced: its types appear in the public signatures. |
