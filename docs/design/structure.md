...
│   │   │                            the one entry schema and its guard, spread by web services,
│   │   │                            static sites, Key Value and Postgres and required on Key Value
│   │   │                            alone (issue #40). Route and Header are not shared — only a
│   │   │                            static site takes them (spec §4.8), so they live in
│   │   │                            static-site.ts with the factory (issue #6)
