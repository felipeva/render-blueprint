...
- Seven prose rules the library enforces beyond the schema, each at the rung the prose earns
  ...
  `ipAllowList` `source` that is neither an address nor a CIDR range (§7), a `diskSizeGB`
  that is neither 1 GB nor a multiple of 5 GB (§9), and high availability on a Postgres plan
  with less than 1 CPU (§9). Warnings: an `autoDeployTrigger` on a service that deploys a
  prebuilt image (§4.3), a `previewValue` on a worker or a cron job (the preview-environments
  page), and a persistence mode other than `off` on a free Key Value instance (§5).
