...
- **Preview generation `off`** — a service's `previews.generation` accepts `off` beside `manual` and `automatic`, following the schema's shared `previewsGeneration` enum rather than raw L476-480, which names only the other two. The prose says "Supported values include" at raw L476, and raw L481 documents an omitted field as disabling previews for that service, so `off` is the value that says what omission does. Not a gap. `src/enums/preview-generation.ts`.
- **Missing static publish path** — a static site with no `staticPublishPath` is a warning, not an issue, although raw L736 calls the field *Required* and the schema leaves it out of `staticService.required`. Render adopts a service by name, and an adopted site may already carry the path, which is the reasoning the missing build command and missing start command warnings already follow. Not a gap. `src/validation/rules/missing-static-publish-path.ts`.
