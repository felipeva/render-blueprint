...
| Add a rule about one config's own values                    | a refinement in that factory's private schema, raised through `raise(ctx, 'Code', …)` so it keeps its own code; one that reads more than one field is attached with `whenFieldsParsed` naming those fields, so a type failure elsewhere does not hide it                                                                                                                                                                                                                                                                     | the factory's `.test.ts`, through `test/support/raised-issues.ts`                                                                                     |
