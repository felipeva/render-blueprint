...
| Add a rule about one config's own values                    | a refinement in that factory's private schema, raised through `raise(ctx, 'Code', …)` so it keeps its own code; one that reads only the field it reports on is attached with `whenFieldsParsed`; one that reads others names every field once in `readingFields`, is attached with its `guard` so a type failure elsewhere does not hide it, and raises through its `raise` so a value a defaults scope supplied is named                                                                                                    | the factory's `.test.ts`, through `test/support/raised-issues.ts`                                                                                     |
