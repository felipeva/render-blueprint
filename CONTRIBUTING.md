...
| Change what YAML comes out, or in what order                | the `_FIELDS` tuple beside the factory, then `src/synth/<kind>.ts`. `src/synth/key-order.ts` only for the orders no resource owns: the root, the placement axes, the env-var entry forms and the registry-credential nodes                                                                                                                                                                                    | `test/fixtures.test.ts`, `test/key-order-conformance.test.ts`, `src/synth/*.test.ts`                                                                  |
